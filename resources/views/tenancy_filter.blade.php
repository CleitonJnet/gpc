...
@section('content')
    ...
    <div id="content_sale">
        <section class="content" id="tenancy">
            ...
            <div class="mt-3">{{ $rants->appends(['tot_all_max'=>$tot_all_max,'tot_all_min'=>$tot_all_min,'type_rent'=>$type_rent,'rooms'=>$rooms,'park'=>$park,'size'=>$size])->links() }}</div>
            <div class="row">
                <div class="col">
                    <span>@if($tot_house == 0) Não foram encontrados imóveis nessas características. @elseif($tot_house > 1) Foram encontrados <strong>{{ $tot_house }}</strong> imóveis. @else Foi encontrado <strong>{{ $tot_house }}</strong> imóvel. @endif</span>
                </div>
            </div>
        </section>
    </div>
